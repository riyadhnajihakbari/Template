        ...
        <div class="lg:col-span-2">
            <div class="card">
                ...
                <div class="flex space-x-2 mb-4 overflow-x-auto">
                    ...
                    <button onclick="filterCategory('{{ $category->id }}')" class="category-btn px-4 py-2 rounded-lg bg-gray-200 text-gray-800 font-semibold whitespace-nowrap hover:bg-gray-300">
                        {{ $category->name }}
                    </button>
                    @endforeach
                </div>
                ...
                <div id="menu-grid" class="grid grid-cols-2 md:grid-cols-3 gap-4">
                    @foreach($categories as $category)
                        ...
                        <div class="menu-item-card" data-category="{{ $category->id }}" onclick='POS.addToCart(@json($item))'>
                            @if($item->photo_url)
                            <img src="{{ asset('storage/' . $item->photo_url) }}" alt="{{ $item->name }}" class="w-full h-32 object-cover rounded-lg mb-2">
                            @else
                            ...
                            <p class="text-pos-primary font-bold">Rp {{ number_format($item->price, 0, ',', '.') }}</p>
                            @if($item->stock > 0)
                            <span class="text-xs text-gray-600">Stok: {{ $item->stock }}</span>
                            @else
